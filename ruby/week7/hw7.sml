...
fun intersect (v1,v2) =
	...
	(case intersect(two_points_to_line seg, v2) of (*延长线段seg, 看起与v2的交集*)
	    NoPoints => NoPoints 
	  | Point(x0,y0) => (* see if the point is within the segment bounds *) (*看点是否在seg上*)
	  ...
	  | LineSegment seg2 => (*形成线段seg2， 所以seg2只可能是线段， 且斜率相等*)
...
fun preprocess_prog (exp) =
	case exp of
		LineSegment(x1, y1, x2, y2) =>
			let 
				fun higherThan(a, b) = not(real_close(a, b)) andalso a > b
			in
				if real_close_point (x1, y1) (x2, y2)
				then Point(x1, y1)
				else if higherThan(x1, x2) orelse (real_close(x1, x2) andalso higherThan(y1, y2))
